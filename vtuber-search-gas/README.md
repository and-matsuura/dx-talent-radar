...
### スプレッドシートにメニューを追加する方法

#### 重要なポイント

**StandaloneスクリプトとContainer-boundスクリプトの違い：**

- **Standaloneスクリプト**（現在のプロジェクト）：
  - `CONFIG.SPREADSHEET_ID`を使ってスプレッドシートにアクセス
  - すべての機能（チャンネル追加、検索、更新など）が動作します
  - メニューを追加するには`createVTuberRadarMenu`を手動実行
  - `onOpen`は自動実行されません（スプレッドシートに紐づいていないため）

- **Container-boundスクリプト**（スプレッドシートに直接紐づいたスクリプト）：
  - そのスプレッドシート専用
  - `onOpen`が自動実行される
  - メニューが自動で表示される

**重要なこと：**
- 「最初に紐づいたスクリプトでしか機能しない」ということは**ありません**
- Standaloneスクリプトからも、スプレッドシートIDを指定すれば同じスプレッドシートにアクセスできます
- 複数のスクリプトから同じスプレッドシートにアクセスすることは可能です
- 違いは、メニューの自動表示がContainer-boundスクリプトでのみ可能ということだけです

スプレッドシートのツールバーに「VTuber Radar」メニューを表示するには、以下のいずれかの方法を使用します：

#### 方法1: Standaloneスクリプトからメニューを追加（簡単）
...
2. `createVTuberRadarMenu`関数を実行
   - 関数を選択して「実行」ボタンをクリック
   - 初回実行時は認証が必要です

3. スプレッドシートを開いて、メニューバーに「VTuber Radar」が表示されることを確認

**注意**: Standaloneスクリプトの場合、スプレッドシートを開くたびに自動でメニューが追加されません。メニューを追加するには、`createVTuberRadarMenu`関数を手動で実行する必要があります。

#### 方法2: スプレッドシートに直接スクリプトを追加（自動実行・推奨）

スプレッドシートを開くたびに自動でメニューを表示したい場合は、スプレッドシートに直接スクリプトを追加します：

1. スプレッドシートを開く（`CONFIG.SPREADSHEET_ID`で指定したスプレッドシート）
2. メニューから「拡張機能」→「Apps Script」を選択
3. 新しいスクリプトエディタが開きます
4. 既存のコードをすべて削除し、`src/SpreadsheetMenu.js`の内容をコピー&ペースト
   - このファイルには、メニュー表示用の最小限のコードが含まれています
5. 保存（Ctrl+S / Cmd+S）
6. スプレッドシートを再読み込みすると、メニューが自動で表示されます

**注意**: この方法の場合、メニューは表示されますが、実際の処理（チャンネル追加など）はStandaloneスクリプト側で実行する必要があります。メニューから実行すると、Standaloneスクリプトでの実行方法が案内されます。

**推奨**: 開発・デプロイはStandaloneスクリプトで行い、スプレッドシートにはメニュー表示用の最小限のコードのみを追加する方法が管理しやすいです。

#### 方法3: スプレッドシートにすべてのコードを追加（完全自動実行）

スプレッドシート側で完全に動作させたい場合は、すべてのコードを追加します：

1. スプレッドシートを開く（`CONFIG.SPREADSHEET_ID`で指定したスプレッドシート）
2. メニューから「拡張機能」→「Apps Script」を選択
3. 新しいスクリプトエディタが開きます
4. 既存のコードをすべて削除し、Standaloneスクリプトのすべてのコードをコピー&ペースト
   - `src/`フォルダ内のすべての`.js`ファイルの内容をコピー
   - または、GASエディタでStandaloneスクリプトのコードをコピー
5. ファイル名を適切に設定（Code.js, Config.js, YouTubeSearch.js など）
6. 保存（Ctrl+S / Cmd+S）
7. スプレッドシートを再読み込みすると、メニューが自動で表示され、すべての機能が動作します

**注意**: この方法の場合、StandaloneスクリプトとContainer-boundスクリプトの2つを管理する必要があります。コードを更新する際は、両方に反映する必要があります。

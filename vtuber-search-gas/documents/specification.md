...
| 情報更新間隔 | 7日 | 既存チャンネルの詳細情報を再取得する間隔（APIクォータ節約） |
...
|------|---------------|
| 新規チャンネル | 検索実行時に追加 |
| 既存チャンネル | 前回更新から7日以上経過後（設定変更可能） |
| 同時接続数 | ライブ配信検出時に随時記録 |

> **💡 ヒント**: 既存チャンネルの更新間隔は `Config.js` の `UPDATE_INTERVAL_DAYS` で変更できます。APIクォータを節約するため、デフォルトは7日に設定されています。
